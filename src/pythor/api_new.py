"""New public API for RTHOR analyses (v0.2.0+)."""
...
def rthor_test(
    ...
    data : Path, str, list[pd.DataFrame], or np.ndarray
        ...
        - **File path** (Path or str): Path to text file containing correlation
          matrices. Requires `n_matrices` and `n_variables` parameters.
        - **DataFrames** (list[pd.DataFrame]): List of DataFrames with raw data.
    ...
    order : str or list[int], default="circular6"
        ...
        - **Custom list**: List of integers specifying pairwise ordering.
          Length must be n*(n-1)/2 where n is number of variables.

    labels : list[str], optional
        Descriptive labels for each matrix. If None, no labels are used.
        Length must equal number of matrices.

    n_matrices : int, optional
        Number of matrices in file (required for file input, ignored otherwise)

    n_variables : int, optional
        Number of variables per matrix (required for file input, ignored otherwise)
    ...
    RTHORResult
        ...
        - **results** (DataFrame): Main results table with columns:
          matrix, predictions, agreements, ties, ci, p_value, label
        - **n_matrices** (int): Number of matrices analyzed
        - **n_variables** (int): Number of variables per matrix
        - **order** (ndarray): The hypothesized ordering used
        - **n_predictions** (int): Number of hypothesized predictions
        - **n_permutations** (int): Number of permutations tested
    ...
    Notes
    -----
    The RTHOR test evaluates whether the pattern of correlations in a correlation
    matrix matches a hypothesized ordering of variables. This is particularly
    useful for testing circumplex or circular models where variables are
    hypothesized to fall along a circular continuum.

    The test uses a randomization approach: the observed Correspondence Index (CI)
    is compared to CIs from permuted data to obtain a p-value. CI ranges from
    -1 (perfect disagreement) to +1 (perfect agreement).
...
def compare_matrices(
    ...
    ----------
    data : Path, str, list[pd.DataFrame], or np.ndarray
        Input data (same formats as `rthor_test`)

    order : str or list[int], default="circular6"
        Hypothesized ordering (same as `rthor_test`)

    n_matrices : int, optional
        Number of matrices (required for file input)

    n_variables : int, optional
        Number of variables (required for file input)
    ...
    ComparisonResult
        ...
        - **rthor_results** (DataFrame): Individual RTHOR tests for each matrix
        - **comparisons** (DataFrame): Pairwise comparison results with columns:
          matrix1, matrix2, both_agree, only1, only2, neither, ci, p_value
        - **n_matrices** (int): Number of matrices analyzed
        - **n_variables** (int): Number of variables per matrix
        - **order** (ndarray): The hypothesized ordering used
        - **n_predictions** (int): Number of hypothesized predictions
    ...
    1. **Individual tests**: Each matrix is tested against the hypothesis
       independently (same as `rthor_test`)

    2. **Pairwise comparisons**: Each pair of matrices is compared to determine
